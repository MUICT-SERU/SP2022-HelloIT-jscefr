...
def write_Results(repo):
    ...
    # Create new folder
    try:
        os.mkdir(wd + "/report/DATA_JSON")
    except FileExistsError:
        pass
    # Create a file for each repository
    name_file = wd + "/report/DATA_JSON/" + repo + '.json'
    repository = dict()
    repository[repo] = dict_total[repo]
    with open(name_file, 'w') as file:
        json.dump(repository, file, indent=4)
    # Create a total file
    name_file = wd + "/report/DATA_JSON/total_data.json"
    with open(name_file, 'w') as file:
        json.dump(dict_total, file, indent=4)
    # Create a summary data
    name_file = wd + "/report/DATA_JSON/summary_data.json"
    with open(name_file, 'w') as file:
        json.dump(dict_summary, file, indent=4)
    # Create a repo data
    name_file = wd + "/report/DATA_JSON/repo_data.json"
    with open(name_file, 'w') as file:
...
def read_Json():
    ...
    with open(current_path + '/report_generators/data.json') as file:
        data = json.load(file)
        extract_Levels(data)
        result = show_Results()
        return result
